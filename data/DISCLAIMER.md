...
> Data was not uploaded due to large file size (+7k images), however the file I used to process the images and later save them in `data/processed` divided in `train` and `val` folders is in `services/inference/utils/convert_split_data.py`.
